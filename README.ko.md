...
- **Composer Control Buttons** — 데스크톱 컴포저의 라이브 상태 색상 토글 버튼 4개 (빨강 = 비활성, 노랑 = 프로비저닝, 초록 = 활성) :
  1. **서브-에이전트 활성화** — 전문화된 AI 서브-에이전트를 활성화하며, 각 에이전트가 자율적으로 하위 작업을 맡고 고유한 전문 능력을 갖추고 작업과 시간 흐름에서 지속적으로 학습합니다.
  2. **비서 (세크리테리)** — 에이전트의 관리자: 작업을 계획하고 할당하며, 커뮤니케이터로서 영화 품질의 오디오(음성 입력/출력)를 제공합니다 (Kokoro `df_eva`, 속도 0.9).
  3. **일시적 복제** — 에이전트가 작업 기간 동안 일시적으로 증식하여 유사한 작업을 동시에 처리할 수 있게 합니다.
  4. **조화와 오케스트레이션** — 작업 내 에이전트를 (비서 없이) 조화/동기화하거나, 비서가 켜져 있을 때는 비서가 주도하여 함께 조화/동기화됩니다。
...
- **학습 중인 크루(라이브 점수)** — 컴포저 아래 막대가 학습 진행을 표시: 상단 행 *Hermes Agent*, *플래너*, *비서*, 하단에 전문가(리서치, 코드, 분석, 이미지, 오디오, 플래닝)가 **최대 2개의 콤팩트한 행**에 — 전문가당 1행이 아닙니다. 각 전문가는 고유한 점수, 의사결정 수, 추세를 가집니다. 실제 값만 표시; 실제 위임 없으면 막대는 비어 있음.
- **모델 선택기** — 드롭다운이 Nous 모델, **OpenRouter**, **MLX-Runtime 네이티브** 카테고리(시스템 드라이브의 로컬 MLX 모델)를 그룹화합니다. 전환은 즉시且安全: 항상 단일 백엔드만 실행(16GB Mac mini RAM 보호), 빠른 전환 시 **마지막 선택**이 항상 승리, 대체된 로드는 깨끗하게 중단.
