...
> **Sprachen:** Diese README ist verfügbar in 32 Sprachen — [Deutsch](README.de.md), [English](README.en.md), [Français](README.fr.md), [Español](README.es.md), [Italiano](README.it.md), [Nederlands](README.nl.md), [Polski](README.pl.md), [Português](README.pt.md), [Русский](README.ru.md), [Türkçe](README.tr.md), [Ελληνικά](README.el.md), [한국어](README.ko.md), [日本語](README.ja.md), [中文](README.zh-CN.md), [繁體中文](README.tw.md), [ไทย](README.th.md), [Tiếng Việt](README.vn.md), [Čeština](README.cz.md), [Slovenčina](README.sk.md), [Magyar](README.hu.md), [Română](README.ro.md), [Suomi](README.fi.md), [Dansk](README.dk.md), [Norsk](README.no.md), [Svenska](README.se.md), [עברית](README.il.md), [हिन्दी](README.in.md), [العربية](README.sa.md), [Українська](README.ua.md), sowie englische Varianten AU/CA/UK/US/ZA, [اردو](README.ur-pk.md) und [RUN (Runic / Elder Futhark)](README.run.md). Die Desktop-App hat einen integrierten Sprachumschalter (🌐) mit all diesen.
...
- **Composer Control Buttons** — vier Umschalt-Buttons im Desktop-Composer mit Live-Statusfarben (rot = inaktiv, gelb = Bereitstellung, grün = aktiv):
  1. **Sub-Agenten aktivieren** — aktiviert die spezialisierten KI-Sub-Agenten, die sich eigenständig Teilaufgaben vornehmen, jeweils mit eigenen Spezialfähigkeiten, und kontinuierlich aus Aufgaben und über die Zeit lernen. Ist die Sekretärin **aus**, arbeitet die Crew autonom: sie holt sich Aufgaben selbst bei Hermes Agent ab (niedrigere Parallel-Schwelle, doppeltes Eigenbudget) und meldet direkt an ihn zurück.
  2. **Secretary / Sekretärin** — die Managerin der Agenten: plant und teilt Aufgaben zu und stellt als Kommunikatorin einen Sprachmodus (Spracheingabe/-ausgabe) mit Audio in Filmqualität bereit (Kokoro `df_eva`, speed 0.9). Sie ist zugleich die **Kommunikationsschnittstelle** zwischen dem Multi-Agenten-System und dem menschlichen Anwender. Wird sie eingeschaltet, aktiviert sie ihre Crew (Button 1) automatisch mit; schaltet man sie wieder aus, bleibt eine **manuell** aktivierte Crew erhalten.
...
- **Orchestration HUD** — vier blau umrandete Live-Felder unter dem Composer-Eingabefeld: *Sub-Agenten-Team*, *Hermes-Sekretärin Auslastung*, *Kopierte Agenten (Cloned Agents)* und *Harmonisierung & Agentenauslastung*. Felder erscheinen nur, wenn eine echte Aufgabe läuft — keine Demo-Platzhalter.
...
- **Letzter Lernerfolg** — eine kompakte Zeile unter dem Composer zeigt das zuletzt erfolgreich gelernte Ergebnis der Sekretärin (welcher Agent sich verbessert hat, Topologie, Klon-Faktor, Einheiten, Latenz) plus ein **Live-MLX-Badge**, das das aktuell von der lokalen Laufzeit auf `:1240` ausgelieferte Modell anzeigt. Die Werte sind echt, von `/secretary-learning` abgerufen — ohne echte Aktivität bleibt das Feld leer.
- **System-RAM-Status** — ein Live-Streifen (pollt `/health` auf `:1240`) zeigt gesamt / frei RAM und Auslastungsprozent, damit du den Spielraum des 16-GB-Mac-mini siehst, während Modelle laden. Versteckt, bis die erste Messung eintrifft.
- **Composer über volle Breite** — das Composer-Eingabefeld spannt jetzt die gesamte Fensterbreite (`--composer-width: 100%`) und gibt langen Prompts und den Live-HUD-Feldern mehr Platz.
